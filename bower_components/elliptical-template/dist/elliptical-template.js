...
(function (root, factory) {
    ...
    dust.onLoad = function (template, callback) {
        if (template.indexOf('/') === 0) {
            template=string.trimFirstChar(template);
            loadTemplateFromFolder(dust, template, callback);
        }else if(template.indexOf('.') > -1){
            loadTemplateFromViewsFolder(dust, template, callback);
        } else {
    ...
    function getSanitizedUrlPath($provider,template){
        var url='';
        var tmpl=template;
        if(tmpl.indexOf('/')===0) tmpl=string.trimFirstChar(tmpl);
        else url = sanitizeFolder($provider._root);
        return getUrlPath(url,tmpl);
    }
