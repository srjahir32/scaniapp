...
(function (root, factory) {
  ...
  dust.helpers.urlDecode = function (chunk, context, bodies, params) {
    var value = dust.helpers.tap(params.value, chunk, context);
    if (value) {
  ...
  dust.helpers.toQueryString = function (chunk, context, bodies, params) {
    var source = dust.helpers.tap(params.value, chunk, context);
    var type = dust.helpers.tap(params.type, chunk, context);
    var id = dust.helpers.tap(params.id, chunk, context);
    if (source === null || source === '') return chunk.write(null);
    if (type === null || type === '') return chunk.write(source);
    if (id === null || id === '') source = source + "?$queryType=" + type;
    else source = source + "?$queryType=" + type + "&$queryId=" + id;

    return chunk.write(source);
  };
